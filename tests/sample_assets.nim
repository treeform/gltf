...
const
  BackendName = when defined(useDirectX): "directx"
    elif defined(useVulkan): "vulkan"
    elif defined(useMetal4): "metal"
    else: "opengl"
  WindowSize = 512
  VerticalFov = 45'f
  FitPadding = 1.25'f
...
<h1>Xray Report</h1>
""" & "<p><b>Native backend:</b> " & BackendName & "</p>" & cardHtml & """
...
""" & (if iblDirectory.len > 0:
  (if BackendName in ["opengl", "directx", "vulkan"]:
    "<p><b>Matched lighting:</b> shared Khronos neutral HDR environment, rotation and exposure; linear sRGB textures, GGX image-based lighting, HDR framebuffer and PBR Neutral tone mapping.</p>"
  else:
    "<p><b>Lighting currently differs:</b> this backend does not yet use the shared HDR environment and tone-map pass.</p>")
...
elif not defined(useVulkan) and not defined(useMetal4):
  if iblDirectory.len > 0:
    pbrContext.attachIblEnvironment(loadIblEnvironment(iblDirectory))
  else:
