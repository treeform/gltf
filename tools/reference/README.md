...
pinned to commit `818318c0b09334998bea39766d88c2d58a76a2f6` on
...
test, plus disabling face culling in the final tone-map pass. That second fix
prevents a mirrored last mesh from hiding the entire frame (VirtualCity).
A third correction decodes specular/glossiness texture RGB as sRGB while
keeping glossiness alpha linear, as required by
[KHR_materials_pbrSpecularGlossiness](https://github.com/KhronosGroup/glTF/blob/main/extensions/2.0/Archived/KHR_materials_pbrSpecularGlossiness/README.md#specularglossinesstexture).
All three corrections have WebGL regression tests. The newer world-space skinning behavior is retained, including ignoring
...
`overview_card.png`, `metrics.json`, `nim-run.log`, `generated/` and `xray/`. It does not regenerate
...
grid positions in the bottom half. It includes only successful comparisons
with valid render and X-ray files, including visual failures. Each file gets
one tile pair: its first valid rest pose, or its first compared capture if no
rest pose is available. Files are ordered by their worst Pixie score across
all captures. Baselines, skipped assets and loading failures are omitted.
The detailed sections still contain every animation frame. Click a tile to
jump to its full comparison, or open the PNG to share the card by itself.
...
git checkout --detach 818318c0b09334998bea39766d88c2d58a76a2f6
npm run build
```
...
five GGX specular mip levels, Charlie sheen mip levels, both reflectance lookup
textures and the sheen energy lookup texture from Khronos's GPU resources.
Linear RGBA float data is saved under `.cache/ibl`, together
...
| --- | ---: |
| Under the existing 2% Pixie threshold | 246 |
| Rendered, above the threshold | 32 |
...
unchanged. All five captures pass, with RGB MAE about 1.283/255 (77.4% of pixels
within ±2). Fine texture differences remain. GlamVelvetSofa uses a nearly black
base color with blue sheen and tinted dielectric reflections. Supporting those
...
IOR and volume thickness displace the sample and absorption tints it. This
matches the pinned reference's buffer format and filtering. Material alpha
still controls coverage: transmission does not turn `OPAQUE` into `BLEND`.

For a scene spread across multiple `draw` calls, wrap those calls in
`pbr.beginPass()` / `pbr.endPass()` inside the IBL frame so every object shares
one snapshot. A single `draw(model)` already does this for the whole hierarchy.
The snapshot is reused as storage, but rebuilt each pass from the current pose.
...
`tests/transmission_pipeline.nim` exercises first/repeated frames, opaque and
blended backgrounds, draw ordering, UV1/transformed data maps, thickness and
absorption, refraction, mip blur, IOR, directional backlighting and shadow-pass
coexistence on the GPU. Run it with `nim c -r tests/transmission_pipeline.nim`.
`tests/test_transmission.nim` runs in the core suite and checks extension
defaults and PNG/KTX2 material round trips, including texture samplers and UVs.
