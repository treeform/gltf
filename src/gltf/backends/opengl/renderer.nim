...
type
  ...
  PbrDrawScratch* = object
    ## Reusable storage for hot PBR draw-path work.
    jointMatrices: seq[Mat4]
    blended: seq[BlendEntry]
    deferred: seq[BlendEntry]

var defaultPbrDrawScratch: PbrDrawScratch
...
proc renderPbrPrimitive(
  ...
  root.skinMatricesInto(owner, scratch.jointMatrices)
  let useSkinning = scratch.jointMatrices.len > 0
  glUniform1i(
  ...
  if useSkinning:
    glUniformMatrix4fv(
      ...
      scratch.jointMatrices.len.GLsizei,
      GL_FALSE,
      cast[ptr float32](scratch.jointMatrices[0].addr)
    )
...
proc drawPbr*(
  ...
  renderPbrNode(
    ...
    shadowTex=0,
    deferBlend=true,
    blended=scratch.blended,
    scratch=scratch,
    root=node
  )

  if scratch.blended.len > 0:
    glDepthMask(GL_FALSE)
    scratch.blended.sort(proc(a, b: BlendEntry): int =
      let
    ...
    for entry in scratch.blended:
      scratch.deferred.setLen(0)
      renderPbrPrimitive(
        entry.primitive,
        entry.transform,
        view,
...
proc drawPbr*(
  ...
  ## Draws a node tree with PBR shading using renderer-owned default scratch.
  node.drawPbr(
    transform,
    view,
    proj,
    tint,
    defaultPbrDrawScratch,
    useTrs=useTrs,
...
proc renderShadowNode(
  ...
  for child in node.nodes:
    renderShadowNode(child, root, node.mat, view, proj, tint, scratch, applyTrs=true)
...
proc drawPbrWithShadow*(
  ...
  ## Draws a node tree with PBR shading and shadows using default scratch.
