...
const ShaderPi = 3.1415926535897932384626433832795'f32
...
proc sampleShadow(posLightSpace: Vec4, normal, lightDir: Vec3): float32 =
  result = 0.0'f32
  if useShadow:
    var projCoords = posLightSpace.xyz / posLightSpace.w
    projCoords = projCoords * 0.5'f32 + vec3(0.5'f32, 0.5'f32, 0.5'f32)
    if projCoords.z <= 1.0'f32 and
        projCoords.x >= 0.0'f32 and projCoords.x <= 1.0'f32 and
        projCoords.y >= 0.0'f32 and projCoords.y <= 1.0'f32:
      let bias = max(shadowBias, 0.002'f32 * (1.0'f32 - dot(normal, lightDir)))
      var
        total = 0.0'f32
        weightSum = 0.0'f32
      for ix in 0 ..< 5:
        for iy in 0 ..< 5:
          let
            x = ix - 2
            y = iy - 2
            wx = 1.0'f32 - abs(x.float32) * 0.25'f32
            wy = 1.0'f32 - abs(y.float32) * 0.25'f32
            w = wx * wy
          ...
          total += w * lit
          weightSum += w
      let litAvg = total / weightSum
      result = 1.0'f32 - litAvg
...
proc gltfPbrFrag*(
  ...
  let
    ...
    metallic =
      ...
      metallicFactor
    transmission = clamp(transmissionFactor, 0.0'f32, 1.0'f32)
    ambientOcclusion =
    ...
    meshNormal: Vec3 =
      if length(normal) > 0.0'f32:
        ...
        geometricNormal
    normalValue: Vec3 =
      normalize(texture(normalTexture, normalUv).rgb * 2.0'f32 -
        vec3(1.0'f32, 1.0'f32, 1.0'f32)) *
      normalScale
  ...
  let
    ...
    nDotL = max(dot(computedNormal, sunDir), 0.0'f32)
    f0 = vec3(0.04'f32, 0.04'f32, 0.04'f32)
    f0mix: Vec3 = mix(f0, albedo, metallic)
    cosTheta = max(dot(computedNormal, viewDir), 0.0'f32)
    fresnel: Vec3 = f0mix +
      (vec3(1.0'f32, 1.0'f32, 1.0'f32) - f0mix) *
      pow(1.0'f32 - cosTheta, 5.0'f32)
    specComponent = nDotL * pow(nDotH, 2.0'f32 / (roughness + 0.0001'f32))
    specular: Vec3 =
      sunLightColor.rgb *
      sunLightColor.a *
      fresnel *
    ...
    diffuse: Vec3 =
      ...
      albedo *
      nDotL
    shadow = sampleShadow(vPosLightSpace, computedNormal, sunDir)
    rim =
      pow(1.0'f32 - max(dot(computedNormal, viewDir), 0.0'f32), 2.5'f32) *
      (1.0'f32 - metallic)
    rimFacing = max(dot(computedNormal, rimDir), 0.0'f32)
    rimLight: Vec3 =
      rimLightColor.rgb *
      rimLightColor.a *
      rim *
      (0.15'f32 + 0.85'f32 * rimFacing) *
      (0.15'f32 + 0.35'f32 * ambientOcclusion)
...
proc gltfSkyboxVert*(
  ...
  gl_Position = vec4(vertexPosition.x, vertexPosition.y, 1.0'f32, 1.0'f32)
  let far = invProj *
    vec4(vertexPosition.x, vertexPosition.y, 1.0'f32, 1.0'f32)
...
proc gltfSkyboxFrag*(rayDir: Vec3, fragColor: var Vec4) =
  let
    ...
    spread = lod * 0.015'f32
  var colorValue = vec4(0.0'f32, 0.0'f32, 0.0'f32, 0.0'f32)
  for i in 0 ..< 16:
    let
      t = i.float32 / 16.0'f32
      angle = t * 2.0'f32 * ShaderPi
      r = sqrt(t)
...
proc gltfShadowDepthVert*(
  ...
  let skin =
    if useSkinning:
      ...
      identityMat4()
  gl_Position = proj * view * model * skin * vec4(vertexPosition, 1.0'f32)

proc gltfShadowDepthFrag*(fragmentUv: Vec2) =
  if alphaCutoff >= 0.0'f32:
