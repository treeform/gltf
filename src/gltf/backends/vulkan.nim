...
proc uploadImage(renderer: VulkanRenderer, image: Image): VkTexture =
  let mips = image.buildImageMips()
  renderer.uploadRgbaSubresources(image.width, image.height, mips.len, 1, mips)

proc uploadSolidImage(renderer: VulkanRenderer, color: ColorRGBX): VkTexture =
  var image = newImage(1, 1)
  image.fill(color)
  renderer.uploadImage(image)

proc uploadStudioCube(renderer: VulkanRenderer): VkTexture =
...
proc uploadShadowPlaceholder(renderer: VulkanRenderer): VkTexture =
  var pixel = 1.0'f32
  var stagingBuffer: VkBuffer
  var stagingMemory: VkDeviceMemory
...
proc createSwapChainImageViews(renderer: VulkanRenderer) =
...
proc createDepthResources(renderer: VulkanRenderer) =
...
proc createReadbackBuffer(renderer: VulkanRenderer, size: IVec2) =
...
proc allocateCommandBuffers(renderer: VulkanRenderer) =
...
proc destroySwapChainResources(renderer: VulkanRenderer) =
...
proc createSwapChainResources(renderer: VulkanRenderer) =
...
proc createDescriptorSetLayouts(renderer: VulkanRenderer) =
...
proc createPipelineLayout(renderer: VulkanRenderer) =
...
proc createFrameDescriptorPool(renderer: VulkanRenderer) =
...
proc releaseTexture(renderer: VulkanRenderer, texture: VkTexture) =
  if texture == nil:
    return
...
proc releaseMaterial(renderer: VulkanRenderer, material: VkMaterial) =
  if material == nil:
    return
...
proc releasePrimitive(renderer: VulkanRenderer, primitive: VkPrimitive) =
  if primitive == nil:
    return
...
proc releaseFrameBuffers(renderer: VulkanRenderer) =
...
proc resetFrameResources(renderer: VulkanRenderer) =
...
proc resize(renderer: VulkanRenderer, size: IVec2) =
...
proc ensurePrimitive(renderer: VulkanRenderer, primitive: Primitive): VkPrimitive =
...
proc captureScreenshot*(renderer: VulkanRenderer): Image =
...
proc clearNode*(renderer: VulkanRenderer, node: Node) =
...
proc shutdown*(renderer: VulkanRenderer) =
