...
proc executeUpload(renderer: DirectXRenderer) =
...
proc uploadImage(renderer: DirectXRenderer, image: Image): DxTexture =
...
proc uploadSolidImage(renderer: DirectXRenderer, color: ColorRGBX): DxTexture =
  var image = newImage(1, 1)
  image.fill(color)
  renderer.uploadImage(image)

proc uploadStudioCube(renderer: DirectXRenderer): DxTexture =
...
proc uploadShadowPlaceholder(renderer: DirectXRenderer): DxTexture =
  var pixel = 1.0'f32
...
proc createDepthBuffer(renderer: DirectXRenderer, size: IVec2) =
...
proc createReadbackBuffer(renderer: DirectXRenderer, size: IVec2) =
...
proc getPipeline(renderer: DirectXRenderer, key: PipelineKey): ID3D12PipelineState =
...
proc createRootSignature(renderer: DirectXRenderer) =
...
proc resize(renderer: DirectXRenderer, size: IVec2) =
...
proc ensurePrimitive(renderer: DirectXRenderer, primitive: Primitive): DxPrimitive =
...
proc captureScreenshot*(renderer: DirectXRenderer): Image =
...
proc clearNode*(renderer: DirectXRenderer, node: Node) =
...
proc shutdown*(renderer: DirectXRenderer) =
